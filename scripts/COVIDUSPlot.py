...
locdf =  (pd.DataFrame.from_dict(get_state_level(x)) for x in get_hopkins())
...
#%%
#My shit attempt at plotly
import plotly.graph_objects as go
df = pd.DataFrame(get_hopkins())
df['text'] = dead['Province/State'] #+ '<br>Population ' + (pops['POPESTIMATE2019']/1e6).astype(str)+' million'
df['name']  = dead['Province/State']
df['lat'] = dead['Lat']
df['lon'] = dead['Long']

df['dead'] = dead.iloc[:, -60:].sum(axis=1)
...
for state in ['Florida']:                       #out.keys():
    ...
    for distanceRate in out[state].keys():
        for key, values in out[state][distanceRate]['SIR'].items():
            fig.add_trace(go.Scatter(x = list(range(len(values))), y = values, mode = 'lines', name = key, visible = False),
            row=1, col=1)
